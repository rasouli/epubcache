...
func NewNotifHandler(config *obj.Config, manager *obj.KeyManager, notifLogger *log.Logger) http.Handler {
	return http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
		...
		if !hashAlreadyExists {
			...
			if uploadedFileIsEpub {
				...
				pdfFileName := fmt.Sprintf("%s%s", notif.Etag, upladedFileFormat)
				pdfDestFile := filepath.Join(storePath, pdfFileName)
				...
				err = attrs.RenderPDF(storePath, pdfFileName, config.CacheSubDirName, pdfCoverName, config.CoverQuality, config.CoverSize)
				if err != nil {
			...
			attributes["_name"] = uploadedFileName
			...
			err = extract.CopyFile(uploadedFileAbsPath,path.Join(metaDir, uploadedFileName))
