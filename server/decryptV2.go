...
func NewDecryptHandlerV2(config *obj.Config, logger *log.Logger) http.Handler {
	return http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			...
			logger.WithFields(log.Fields{"host": req.Host, "body": string(bb), "error": err}).Error("made invalid request to the decrypt v2 api.")
		...
		// previous version used etag, here we use custom uuid to avoid file collision
		uuid, _ := uuid.NewV4()
		decryptedFileName := fmt.Sprintf("%s%s", uuid.String(), decryptedFileFormat)

		encryptedFileSrcPath := msg.Path
